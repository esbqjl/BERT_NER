...
class Vocab(object):
    ...
    def build_vocab(self, texts=None, labels=None, build_texts=True, build_labels=True, with_build_in_tag_id=True):
        ...
        if build_labels:
            ...
            if os.path.isfile(file_path):
                df = pd.read_csv('./data/label.txt', header=None, names=['label', 'id'])
                self.tag2id = df.set_index('label').to_dict()['id']
                self.id2tag = df.set_index('id').to_dict()['label']
    ...
    """
    当使用预训练的模型或Embedding时，可能使用别人的vocab，
    下面set方法允许设置外部的vocab
    """
